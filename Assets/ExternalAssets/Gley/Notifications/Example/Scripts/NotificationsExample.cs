...
namespace Gley.Notifications.Internal
{
    public class NotificationsExample : MonoBehaviour
    {
        ...
        bool sendRepeatNotification = true;
        ...
        public void SendNotificationForToggle(bool value)
        {
            int.TryParse(input.text, out notificationTime);
            if (notificationTime > 0)
            {
                Debug.Log($"{nameof(SendNotificationForToggle)} : {value}");
        ...
        public void SendRepeatNotificationForToggle(bool value)
        {
            int.TryParse(input.text, out repeatNotificationTime);
            if (repeatNotificationTime > 0)
            {
                Debug.Log($"{nameof(SendRepeatNotificationForToggle)} : {value}");
                sendRepeatNotification = value;
            }
        }
        ...
        private void OnApplicationFocus(bool focus)
        {
            if (focus == false)
            {
                Gley.Notifications.API.SendNotification("Game Title", "App was minimized", new System.TimeSpan(0, 0, 1), "icon_0", "icon_1", "Opened from Gley Minimized Notification");
                if (sendNotification)
                {
                    Gley.Notifications.API.SendNotification("Game Title", "Notification", new System.TimeSpan(0, 0, notificationTime), "icon_0", "icon_1", "Opened from Gley Notification");
                }
                if (sendRepeatNotification)
                {
                    Gley.Notifications.API.SendRepeatNotification("Game Title", "Repeat Notification", new System.TimeSpan(0, 0, repeatNotificationTime), new System.TimeSpan(0, 0, repeatNotificationTime), "icon_0", "icon_1", "Opened from Gley Repeat Notification");
                }
